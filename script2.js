...
function checkWinner(a,b,c){
    ...
    b--;
    c--;
    if((span[a].dataset.player === span[b].dataset.player) && (span[b].dataset.player === span[c].dataset.player) && (span[a].dataset.player === span[d].dataset.player) && (span[a].dataset.player ==="x" || span[a].dataset.player === "o") && isGameOver == false){
        span[a].parentNode.className += " activateBox";
        span[b].parentNode.className += " activateBox";
        span[c].parentNode.className += " activateBox";
        isGameOver(a);
    } 
}

function playAgain() {
    document.getElementsByClassName("alert")[0].paraentNode.removeChild(document.getElementsByClassName("alert")[0]);
    resetgame();
    window.isGameOver = false;
    for(var k =0; k<span.length();k++){
        span[k].parentNode.className = span[k].parentNode.className.replace("activateBox", " ");
    }
}
